...
export default class App extends Component {
	...
	sortDate= (list) => {
		...
		let dateTo = this.state.dateTo;
			list.filter(el => {
				return el.taskByDate = dateTo.length > 1 ? dateTo >= el.tasks.date && el.tasks.date >= dateFrom : el.tasks.date >= dateFrom;
			})
	}
	...
	sortTasks = (sortBy, dir) => {
	 ...
	 sortByName = (sortBy) => {
		...
		list.sort((a, b) => {
			if (name === 'done') {
				let num = (String(a[name]) < String(b[name])) ? 1 : (String(a[name]) > String(b[name])) ? -1 : 0;
				...
				if (a.tasks[name] === b.tasks[name]) {
					...
					changeList = true;
					if ((a.tasks[name] === 'high' && b.tasks[name] === 'medium') || (a.tasks[name] === 'medium' && b.tasks[name] === 'low')) return -1;

					else return 1;
				}

			} else {
				let num = (a.tasks[name] < b.tasks[name]) ? 1 : (a.tasks[name] > b.tasks[name]) ? -1 : 0;
				changeList = (num !== 0) ? true : changeList;
				return num;
			}
		});
  ...
  render() {
    return (
			<div className="app-wrap">
				...
				<FilterTasks toggleShowDone={this.toggleShowDone} tasks={this.filterTasks()} sortTasks={this.sortByName} selectDate={this.changeDate} />
				<TaskList tasks={this.filterTasks()} toggleDone={this.toggleDone} sortTasks={this.sortTasks} />
			</div>
    );
  }
}
